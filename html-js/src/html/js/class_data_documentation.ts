...
type CompressedDataHolder = DocWrapper & {
    data: EntityData;
    getRawType?: () => number | null | undefined;
    getTypeVariableMap?: () => Record<string, unknown>;
    getTypeVariables?: () => unknown[];
    type?: () => number;
    dataIndex?: () => number;
    getDataIndex?: () => number;
    ...
    getDeclaringConstructorWrapped?: () => DocWrapper | null;
    getDeclaringMethodWrapped?: () => DocWrapper | null;
    getWrappedParameterHolder?: (sourceMethodId: number | null, sourceConstructorId: number | null) => DocWrapper | null;
    withTypeVariableMap?: (map: Record<string, unknown>) => Record<string, unknown>;
};
 ...
 */
function setRemapType(target: CompressedDataHolder): DocWrapper {
    const holder = target;
    /**
     ...
     */
    holder.type = function () {
        const type = holder.data[PROPERTY.TYPE] as number;
        if (!exists(holder.getTypeVariableMap)) {
            return type;
        }
        const typeMap = holder.getTypeVariableMap!() as Record<number, number>;
     ...
     */
    holder.getTypeWrapped = function () {
        const result = getClass(holder.type!());
        if (result === null) {
            throw new Error('Type not found for object.');
        }
        result.withTypeVariableMap(holder.getTypeVariableMap!());
        return result;
    };
 ...
 */
function setModifiers(target: CompressedDataHolder): DocWrapper {
    const holder = target;
    /**
     ...
     */
    holder.modifiers = function () {
        if (holder.data[PROPERTY.MODIFIERS] === undefined) {
            if (holder.getRawType !== undefined) {
                const rawTypeId = holder.getRawType();
                if (exists(rawTypeId)) {
        ...
        return holder.data[PROPERTY.MODIFIERS] as number;
    };
     ...
     * @type {(function(): int)}
     */
    holder.getModifiers = holder.modifiers;
...
function setParameters(
    target: CompressedDataHolder,
    sourceClassId: number,
    sourceMethodId: number | null = null,
    sourceConstructorId: number | null = null
): DocWrapper {
    target.getWrappedParameterHolder = function (sourceMethodId: number | null, sourceConstructorId: number | null) {
     ...
     */
    function mapParameter(parameter: number, index: number) {
        const holder = target.getWrappedParameterHolder!(
            sourceMethodId,
            sourceConstructorId
        ) as CompressedDataHolder;
        return getParameter(
            parameter,
    ...
    target.parameters = function () {
        ...
        target.data._parameter_cache = getAsArray(target.data[PROPERTY.PARAMETERS] as number[] | number).map(mapParameter);
        return target.data._parameter_cache;
    }
 ...
 */
function setAnnotations(target: CompressedDataHolder): DocWrapper {
    /**
    ...
    target.annotations = function () {
        ...
        target.data._annotation_cache = getAsArray(target.data[PROPERTY.ANNOTATIONS] as number[] | number).map(mapAnnotation);
        return target.data._annotation_cache;
    }
 ...
 */
function setTypeVariableMap(target: CompressedDataHolder): DocWrapper {
    /**
     * Retrieves the type variable map of the object.
     *
     * @returns {TypeVariableMap} The type variable map of the object.
     */
    target.getTypeVariableMap = function (): Record<string, unknown> {
        return {};
    }
     ...
     */
    target.withTypeVariableMap = function (map: Record<string, unknown>) {
        if (!exists(map) || Object.keys(map).length === 0) {
            return target.getTypeVariableMap!();
        }
        const leftMap = JSON.stringify(target.getTypeVariableMap!());
        const rightMap = JSON.stringify(map);
        if (leftMap === rightMap) {
            return target.getTypeVariableMap!();
        }
        const originalGetTypeVariableMap = target.getTypeVariableMap!.bind(target);
 ...
 */
function setTypeVariables(target: CompressedDataHolder): DocWrapper {
    /**
    ...
    target.getTypeVariablesMapped = function () {
        ...
        return target.getTypeVariables!().map((tv) => typeVariableMap[String(tv)] ?? tv);
    }
 ...
 */
function setBasicName(target: CompressedDataHolder): DocWrapper {
    /**
    ...
    target.name = function () {
        if (!exists(target.data._name_cache)) {
            target.data._name_cache = getNameData(target.data[PROPERTY.NAME] as number | string | number[]);
        }
        return target.data._name_cache as string;
    }
 ...
 */
function setDataIndex(target: CompressedDataHolder): DocWrapper {
    /**
    ...
    target.dataIndex = function () {
        return target.data._dataIndex as number;
    }
 ...
 */
function setDeclaringClass(target: CompressedDataHolder): DocWrapper {
    /**
 ...
 */
function setDeclaringMethod(target: CompressedDataHolder): DocWrapper {
    /**
    ...
    target.getDeclaringMethodWrapped = function () {
        ...
        const declaringClass = getClass(target.getDeclaringClass!());
        if (declaringClass === null) {
            throw new Error(`Declaring class not found for object with data index ${target.getDataIndex!()}`);
        }
 ...
 */
function setDeclaringConstructor(target: CompressedDataHolder): DocWrapper {
    /**
 ...
 */
function setDeclaringParameter(target: CompressedDataHolder): DocWrapper {
    /**
 ...
 */
function setDeclaringField(target: CompressedDataHolder): DocWrapper {
    /**
 ...
 */
function setExceptions(target: CompressedDataHolder): DocWrapper {
    /**
