...
type AddGraphEdgeFn = (fromId: number, toId: number, label: string, metaData?: Record<string, unknown>) => void;
...
function findBrokenClassNames() {
    let brokenNames: [CompressedTypeEntry, number, unknown][] = [];
    DATA.types.forEach((data, index) => {
        try {
            getClass(index)!.name();
        } catch (e) {
...
function findAllTypeVariablesWithInvalidState() {
    ...
    DATA.types.forEach((data, index) => {
        ...
        if (!exists(data[PROPERTY.TYPE_VARIABLES]))
            return;
        let actualTypes = getAsArray(data[PROPERTY.TYPE_VARIABLES] as number | number[]);
        let typeVariables = getClass(data[PROPERTY.RAW_PARAMETERIZED_TYPE] as number)!.getTypeVariables();
...
function findWeirdestNames() {
    let weirdNames: [string, CompressedTypeEntry, number][] = [];
    DATA.types.forEach((data, index) => {
        try {
...
function getParameterizedClasses() {
    let parameterizedClasses: [unknown, number][] = [];
    DATA.types.forEach((data, index) => {
        if (getClass(index)!.isParameterizedType()) {
...
function getWildcardClasses() {
    let wildcardClasses: [unknown, number][] = [];
    DATA.types.forEach((data, index) => {
...
function getTypeVariables() {
    let typeVariables: [unknown, number][] = [];
    DATA.types.forEach((data, index) => {
...
function getProperNameOfRawClass(id: number) {
    ...
    const config = new name_parameters()
        .setTypeVariableMap(typeVariableMap)
        .setIncludeGenerics(false);
    return getGenericDefinitionLogic(classType, config);
}

function getTypeVariableTree(id: number): Record<string, unknown> {
    const classType = getClass(id);
    if (classType == null) {
    ...
    output.id = id;
    output.name = classType.fullyQualifiedName();
    ...
    if (classType.isTypeVariable()) {
        output.bounds = classType.getTypeVariableBounds().map((bound) => {
            ...
            return {
                id: boundType.id(),
                name: boundType.fullyQualifiedName()
            };
        });
        return output;
    }
    ...
    if (classType.isParameterizedType()) {
        ...
        output.actualTypes = classType.getTypeVariables().map((typeVar) => {
            let actualType = getClass(typeVar)!;
            return {
                id: actualType.id(),
                name: actualType.fullyQualifiedName()
            };
        });
    ...
    output.interfaces = classType.getInterfaces().map((interfaceId) => {
        return getTypeVariableTree(interfaceId as number);
    });
...
function createGraphVizGraph(ids: number[]) {
    let graph: GraphVizGraph = {
        nodes: [],
        edges: []
    };
    let seenNodes = new Set<number>();
    let edgedNodes = new Set<number>();
    let seenEdges: Record<number, Set<number>> = {};
    let isolatedNodes = new Set<number>();
    function addNode(id: number, label: string, metaData: Record<string, unknown> = {}) {
    ...
    function addEdge(fromId: number, toId: number, label: string, metaData: Record<string, unknown> = {}) {
...
function addRawClass(type: JavaType, addEdge: AddGraphEdgeFn) {
    if (!type.isRawClass()) {
    ...
    type.getInterfaces().forEach((interfaceId) => {
        addEdge(type.id(), interfaceId as number, "implements");
    });

    // Add Type Variable edges
    type.getTypeVariables().forEach((typeVarId) => {
        addEdge(type.id(), typeVarId, `defines type variable`);
    });
}

function addParameterizedType(type: JavaType, addEdge: AddGraphEdgeFn) {
...
function addTypeVariable(type: JavaType, _addEdge: AddGraphEdgeFn) {
    if (!type.isTypeVariable()) {
        throw new Error("Type is not a type variable. Cannot add type variable edges.");
    }
}

function addWildcardType(type: JavaType, addEdge: AddGraphEdgeFn) {
    if (!type.isWildcard()) {
...
function gatherReferences(id: number, references = new Set<number>()) {
    const classType = getClass(id)!;
    ...
    if (classType.isRawClass()) {
        ...
        classType.getInterfaces().forEach((interfaceId) => {
            gatherReferences(interfaceId as number, references);
        });
...
function generateGraphVizDot(graph: GraphVizGraph) {
    let dot = "digraph G {\n";
...
function generateMermaidGraph(graph: GraphVizGraph) {
...
function generateMermaidGraphFromReferencesToId(id: number) {
...
function generateGraphVizDotFromReferencesToId(id: number) {
    const references = gatherReferences(id);
    const graph = createGraphVizGraph(Array.from(references));
...
function findCircularReferences(
    ...
    let cycle: [number[], number] | null = null;
    if (classType.isRawClass()) {
        if (exists(classType.getSuperClass())) {
        ...
        for (let interfaceId of classType.getInterfaces()) {
            cycle = findCircularReferences(interfaceId as number, visited, path);
            if (cycle) return cycle;
        }
