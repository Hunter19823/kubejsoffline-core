function decompressString(compressedString: number) {
    return DATA.names[compressedString];
}

function getNameData(id: number | string | number[]): string {
    if (!exists(id)) {
    ...
    return getNameData(DATA.names[id] as number | string | number[]);
}
